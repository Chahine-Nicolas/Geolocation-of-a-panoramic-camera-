...
@nb.jit
def convertToFishEye(src, outputSize):
    out = np.ones((outputSize, outputSize, 3), dtype=np.uint8)*255
    w, h, _ = src.shape
...
def createFishEyeIndex(outputSize):
    print("createFishEyeIndex - begin")
...
def unstitchGopro(f0, f5, outputSize, index=None):
    ...
    if index is None:
        ...
        out2 = np.rot90(convertToFishEye(o2, outputSize))
    else:
        out1 = convertToFishEyeFromIndex(o1, index)
        out2 = np.rot90(convertToFishEyeFromIndex(o2, index))
