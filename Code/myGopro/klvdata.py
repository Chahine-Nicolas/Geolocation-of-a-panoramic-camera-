...
class KLVData:
    ...
    def __init__(self, data, offset):
        ...
        self.data = fourCC.myManage(self)  # fourCC.Manage(self)
    ...
    def __str__(self):
        ...
        s = "fourCC=%s type=%s size=%d repeat=%s data={%s} raws=|%s| raw=[%s]"\
            % (self.fourCC, stype, self.size,
               self.repeat, self.data, rawdatas, rawdata)
    ...
    def scaledData(self, scale):
        if isinstance(scale, list):
            for q in self.data:
                yield [float(x) / float(y) for x, y in zip(q, scale)]
        else:
            for q in self.data:
                yield [float(x) / float(scale) for x in q]

    def readTime(self):
        s = self.rawdata.decode('utf-8', errors='replace')
